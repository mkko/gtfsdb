  ...
  function downloadGTFS(task, cb) {
      ...
      if (file_protocol === 'http:' || file_protocol === 'https:') {
        ...
        function processFile(e, response, body) {
          ...
          if (!lastModifiedHeader) {
            ...
            log(agency_key + ': Last modified: Not available');
          } else {
            lastModified = new Date(lastModifiedHeader);
            log(agency_key + ': Last modified: ' + lastModified);
          }
    ...
    function handleImport(cb) {
      ...
      knex.transaction(function(trx) {
        ...
        //.then(importTransfers)
        .then(function() {
          // Finally update the import date.
          return trx("agency").where({ agency_entry_id: agencyEntryId })
          .update({ last_update: lastModified })
        });
      })
      ...
      function removeAgency(trx) {
        ...
        return trx('agency_entry').select('id').where({
          ...
          if (!agencyEntryId) {
            log(agency_key + ': No previous entries for the agency key')
            return;
          } else {
            log(agency_key + ': Removing previous entries with the same agency key');
